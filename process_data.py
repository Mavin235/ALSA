...
def print_features(train_features, tokenized_train_examples, test_features, tokenized_test_examples, n):
    for i in range(n):
        print("*** Example-%d ***"%(i))
        ...
        if "train" in filename:
            with open(filepath, 'r', encoding="UTF-8") as f:
                ...
                while line and line != '':
                    ...
                    if cnt % 3 == 0:
                        ...
                        input_ids, input_masks, segment_ids, tokens_whole \
                            = tokenize(example, tokenizer, args.max_seq_len)
                        train_features.append(Feature(input_ids, input_masks, segment_ids, convert_label_id(example[2])))
            ...
            with open(filepath, 'r', encoding="utf-8") as f:
                ...
                while line and line != '':
                    ...
                    if cnt % 2 == 0:
                        example[0] = example[0].replace("$T$", example[1], 1)             
                        example.append("0") # 由于BERT模型的输入必须要有一个label_id, 所以把所有
                                            # 测试集中样本的label_id都设成 "0" (neual), 在使用中
                                            # 不会用到
                        input_ids, input_masks, segment_ids, tokens_whole \
                            = tokenize(example, tokenizer, args.max_seq_len)
                        test_features.append(Feature(input_ids, input_masks, segment_ids, convert_label_id(example[2])))
